...
export function FacturiCard({ orgSlug, pageId, facturi }: { orgSlug: string; pageId: string; facturi: InvoiceRow[] }) {
  ...
  return (
    <Card>
      ...
      <form action={formAction} className="grid grid-cols-2 gap-2 rounded-[var(--ci-radius-card)] border border-[var(--ci-border)] p-3 sm:grid-cols-3" encType="multipart/form-data">
        <input name="denumire" required placeholder="Denumire document" className="rounded-[var(--ci-radius-btn)] border border-[var(--ci-border)] bg-[var(--ci-surface)] px-3 py-2 text-sm" />
        <input name="suma" type="number" min={1} step={1} required placeholder="Sumă (lei)" className="rounded-[var(--ci-radius-btn)] border border-[var(--ci-border)] bg-[var(--ci-surface)] px-3 py-2 text-sm" />
        <select name="categorie" defaultValue="factura" className="rounded-[var(--ci-radius-btn)] border border-[var(--ci-border)] bg-[var(--ci-surface)] px-3 py-2 text-sm">
        ...
        <select name="status" defaultValue="achitata" className="rounded-[var(--ci-radius-btn)] border border-[var(--ci-border)] bg-[var(--ci-surface)] px-3 py-2 text-sm">
          <option value="achitata">Achitată</option>
          <option value="in_asteptare">În așteptare</option>
        </select>
        <input name="fisier" type="file" required accept=".jpg,.jpeg,.png,.webp" className="col-span-2 text-[12px] text-[var(--ci-text-muted)] file:mr-2 file:rounded-[var(--ci-radius-btn)] file:border-0 file:bg-[var(--ci-surface-2)] file:px-2.5 file:py-1.5 file:text-[12px] file:font-medium file:text-[var(--ci-text)] sm:col-span-1" />
      ...
      <div className="mt-4">
        {facturi.length ? (
          <div className="flex flex-col gap-2">
            {facturi.map((f) => (
              <div key={f.id} className="flex items-center justify-between gap-2 rounded-[var(--ci-radius-card)] border border-[var(--ci-border)] px-3.5 py-2.5">
